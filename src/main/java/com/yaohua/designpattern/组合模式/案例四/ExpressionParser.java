...
public class ExpressionParser {
    ...
    public ExpressionParser(String infixExpression, Map<String,Integer> variablesMap) {
    ...
    public List<String> toSuffix(){
        ...
        while (pointer < infixExpression.length()){
            ...
            }else if (isDigit(infixExpression)){ //5. 遇到数字，加入结果；
               StringBuilder stringBuilder = new StringBuilder();
               while (pointer < infixExpression.length() && isDigit(infixExpression)){
    ...
    public Expression toExpression(){
        ...
        for (String item : suffix) {
            ...
            }else if (variablesMap.containsKey(item)){
                stack.addLast(new NumberExpress(variablesMap.get(item)));
            }else{
                //数字
    ...
    //判断是不是数字
    private boolean isDigit(String infixExpression){
        char c = infixExpression.charAt(pointer);
        if (c == '+' || c == '-' || c == '*' || c == '/' || c == '(' || c == ')') {
           return false;
        }
